...
//  배열에 데이터 삽입
    public void insertData(int index, int data) {
        ...
        // 들어가야하는 데이터의 인덱스 전까지는 그대로 넣고
        for (int i = 0; i < index; i++) {
            this.arr[i] = arrDup[i];
        }
        // 인덱스보다 한칸 뒤부터는 한칸 뒤로 밀어서 넣기.
...
//  배열에서 특정 데이터 삭제
    public void removeData(int data) {
        ...
        // 기존의 배열에 데이터가 있는지 먼저 확인 후 타겟 인덱스 지정해주기
        ...
        // 타겟인덱스가 -1이면 배열에 데이터가 없다는 뜻이므로 없다는 문구 출력
        if (targetIndex == -1) {
            System.out.println("해당 데이터가 없습니다.");
        } else {
            // 있다면 기존 배열보다 크기가 하나 작은 배열을 만들고
            int[] arrDup = this.arr.clone();
            this.arr = new int[this.arr.length - 1];
            ...
            // 타겟 인덱스는 넣지 않고(빼고), 그 뒤는 한칸씩 당겨서 넣어주기
