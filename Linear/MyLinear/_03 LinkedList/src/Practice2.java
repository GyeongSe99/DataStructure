...
class DoublyLinkedList extends LinkedList {
    ...
    // 생성자
    // 처음에는 헤드와 테일이 같은 노드를 가르킬 수 있도록한다.
        ...
        if (this.head == null) {
            ...
            while (cur != null) {
                if (cur.data == beforeData) {
                    if (cur == this.head) { // 넣을려는 위치가 헤드 앞일때,
    ...
    public void removeData(int data) {
        ...
        while (cur != null) {
            if (cur.data == data) {
                // 데이터를 찾았는데 그 데이터가 헤드와도 같고 테일과도 같을때 -> 노드가 하나일때
                if (cur == this.head && cur == this.tail) {
                    this.head = null;
                    this.tail = null;
                } else if (cur == this.head) {  // 데이터가 헤드일때,
                    this.head = cur.next;
                    this.head.prev = null;
                } else if (cur == this.tail) {  // 데이터가 테일일때
